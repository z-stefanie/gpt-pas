...
def metric_cal(scores, gt_list, gt_mask_list, cal_pro=False):
    '''
        ...
        Dimension:
            scores: [#][W][H]
            gt_list: [#] -> image-level
            gt_mask_list: [#][W][H] -> pixel-level

            img_socres: [#][W][H] -> [#][WxH] -> [#] == gt_list
            scores.flatten: [#xWxH]
            
    '''
    gt_mask = np.asarray(gt_mask_list, dtype=int)  
    # scores.shape()
    assert scores.shape == gt_mask.shape, "The dimension of scores and gt_mask should both be [#][W][H]"


    # calculate image-level ROC AUC score
    img_scores = scores.reshape(scores.shape[0], -1).max(axis=1) # #Note: img_scores: [#][W][H] -> [#][WxH] -> [#]
    gt_list = np.asarray(gt_list, dtype=int) # Note: gt_list: [#][1]
    ...
    try:
        ...
        end_time = time.time()
        print(f"p_roc: {end_time-start_time} seconds")
    except Exception as e:
        print(f"Error calculating p_roc: {e}")
        raise RuntimeError(f"Error calculating p_roc: {e}") from e
    
    try:
        start_time = time.time()
        img_f1, img_threshold = calculate_max_f1(gt_list, img_scores)
        end_time = time.time()
        print(f"i_f1: {end_time-start_time} seconds")
    except Exception as e:
            print(f"Error calculating i_f1: {e}")
            raise RuntimeError(f"Error calculating i_f1: {e}") from e
    
    try:
        # Note: pixel-level evaluation below
        start_time = time.time()
        pxl_f1, pxl_threshold = calculate_max_f1(gt_mask.flatten(), scores.flatten())
        end_time = time.time()
        print(f"p_f1: {end_time-start_time} seconds")
    except Exception as e:
        print(f"Error calculating p_f1: {e}")
        raise RuntimeError(f"Error calculating p_f1: {e}") from e
    ...
    try:
        ...
        end_time = time.time()
        print(f"i_roc: {end_time-start_time} seconds")
    except Exception as e:
            print(f"Error calculating i_roc: {e}")
            raise RuntimeError(f"Error calculating i_roc: {e}") from e
    ...
    if cal_pro:
        ...
        try:
            ...
            end_time = time.time()
            print(f"p_pro: {end_time-start_time} seconds")
        except Exception as e:
            print(f"Error calculating p_pro: {e}")
            raise RuntimeError(f"Error calculating p_pro: {e}") from e
        ...
        try:
            ...
            end_time = time.time()
            print(f"r_f1: {end_time-start_time} seconds")
        except Exception as e:
            print(f"Error calculating r_f1: {e}")
            raise RuntimeError(f"Error calculating r_f1: {e}") from e
...
def rescale(x):
    ...
    if max_val - min_val == 0:
        raise ValueError("The maximum and minimum values are the same, resulting in a division by zero.")
